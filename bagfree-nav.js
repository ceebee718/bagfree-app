...
(function () {
  ...
  function svg(name, fill) {
    return '<svg class="bf-ic" viewBox="0 0 24 24" fill="' + (fill || 'none') +
  ...
  var css = ''
  + '.bf-sidebar{position:fixed;left:0;top:0;bottom:0;width:240px;z-index:95;background:#0d1a32;'
  ...
  + '.bf-city-info{flex:1;min-width:0;display:flex;flex-direction:column;gap:.12rem}'
  + '.bf-city-label{font-size:.55rem;letter-spacing:2.5px;text-transform:uppercase;color:#c9a96e;font-weight:600}'
  + '.bf-city-name{font-size:.88rem;color:#fdfcf8;font-weight:400;display:flex;align-items:center;gap:.35rem}'
  + '.bf-city-name svg{width:11px;height:11px;color:#c9a96e;flex-shrink:0}'
  + '.bf-city-chev{width:14px;height:14px;color:rgba(253,252,248,0.38);flex-shrink:0}'
  + '.bf-nav{display:flex;flex-direction:column;gap:.12rem}'
  + '.bf-item{display:flex;align-items:center;gap:.85rem;padding:.72rem .9rem;border-radius:9px;color:rgba(253,252,248,0.62);'
  +   'font-size:.92rem;font-weight:400;letter-spacing:.2px;text-decoration:none;border:1px solid transparent;transition:all .2s}'
  + '.bf-item:hover{background:rgba(201,169,110,0.05);color:#c9a96e}'
  + '.bf-item.bf-active{background:rgba(201,169,110,0.08);color:#c9a96e;border-color:rgba(201,169,110,0.18)}'
  + '.bf-item .bf-ic{width:18px;height:18px;flex-shrink:0;opacity:.9}'
  + '.bf-item.bf-active .bf-ic{opacity:1}'
  + '.bf-item[data-icon="star"] .bf-ic{fill:#c9a96e;stroke:#c9a96e}'
  + '.bf-item.bf-sage{background:linear-gradient(135deg,#e6cd92,#c9a96e);color:#3a2c12;border-radius:999px;'
  +   'padding:.72rem 1.25rem;font-weight:500;box-shadow:0 4px 14px rgba(201,169,110,0.4),inset 0 1px 0 rgba(255,255,255,0.38)}'
  + '.bf-item.bf-sage:hover{background:linear-gradient(135deg,#f0d8a0,#d4b87a);color:#2a2008;box-shadow:0 7px 20px rgba(201,169,110,0.55),inset 0 1px 0 rgba(255,255,255,0.42)}'
  + '.bf-item.bf-sage .bf-ic{opacity:1}'
  + '.bf-item.bf-sage .bf-ic img,.bf-item.bf-sage .bf-ic svg{filter:brightness(0);opacity:.82;width:18px;height:18px}'
  + '.bf-spacer{flex:1;min-height:.4rem}'
  + '.bf-rewards{background:linear-gradient(165deg,rgba(201,169,110,0.10),rgba(201,169,110,0.02));border:1px solid rgba(201,169,110,0.16);'
  +   'border-radius:12px;padding:1.2rem 1.1rem;display:flex;flex-direction:column;gap:.5rem}'
  + '.bf-rw-label{display:flex;align-items:center;gap:.4rem;font-size:.55rem;letter-spacing:2.5px;text-transform:uppercase;color:#c9a96e;font-weight:600}'
  + '.bf-rw-label svg{width:13px;height:13px}'
  + '.bf-rw-points{font-family:"Cormorant Garamond",Georgia,serif;font-size:2.15rem;color:#fdfcf8;line-height:1.05}'
  + '.bf-rw-sub{font-size:.72rem;color:rgba(253,252,248,0.38)}'
  + '.bf-rw-link{font-size:.58rem;letter-spacing:2px;text-transform:uppercase;font-weight:600;color:#c9a96e;text-decoration:none;'
  +   'border-top:1px solid rgba(253,252,248,0.06);padding-top:.65rem;margin-top:.15rem;display:flex;align-items:center;gap:.35rem}'
  + '.bf-scrim{position:fixed;inset:0;background:rgba(5,10,22,0.55);z-index:94;opacity:0;visibility:hidden;transition:opacity .28s,visibility .28s}'
  + '.bf-burger{position:fixed;top:.85rem;left:.85rem;z-index:96;width:42px;height:42px;border-radius:10px;'
  ...
  + '.bf-burger svg{width:22px;height:22px}'
  + '@media(min-width:981px){body.bf-shift{padding-left:240px!important}}'
  + '@media(max-width:980px){'
  +   '.bf-sidebar{transform:translateX(-100%);box-shadow:0 0 50px rgba(0,0,0,0.5)}'
  +   '.bf-sidebar.bf-open{transform:translateX(0)}'
  +   '.bf-scrim.bf-open{opacity:1;visibility:visible}'
  ...
  function itemHTML(it) {
    ...
    return '<a class="bf-item' + (it.sage ? ' bf-sage' : '') + (active ? ' bf-active' : '') +
  ...
  var html = ''
    ...
    +   '<span class="bf-city-chev">' + svg('chev') + '</span></a>'
    + '<nav class="bf-nav" aria-label="Primary">' + NAV.map(itemHTML).join('') + '</nav>'
    + '<div class="bf-spacer"></div>'
    + '<div class="bf-rewards">'
    +   '<span class="bf-rw-label">' + svg('crown') + 'BAG Rewards</span>'
    +   '<span class="bf-rw-points" id="bf-points">' + DEFAULT_POINTS + '</span>'
    +   '<span class="bf-rw-sub">Points Available</span>'
    +   '<a class="bf-rw-link" href="' + REWARDS_HREF + '">View Rewards ' + svg('chev') + '</a>'
    + '</div>';

  /* ── Inject ────────────────────────────────────────────────────────────── */
  var style = document.createElement('style');
  style.id = 'bf-nav-style';
  ...
  scrim.className = 'bf-scrim';

  var burger = document.createElement('button');
  burger.className = 'bf-burger';
  ...
  document.body.classList.add('bf-shift');

  function open() { aside.classList.add('bf-open'); scrim.classList.add('bf-open'); }
  function close() { aside.classList.remove('bf-open'); scrim.classList.remove('bf-open'); }
    ...
    try {
      ...
      sb.auth.getSession().then(function (r) {
        ...
        sb.from('profiles').select('preferred_city,bag_balance').eq('id', s.user.id).single()
          .then(function (p) {
            if (!p || !p.data) return;
            var d = p.data, c = document.getElementById('bf-city'), pts = document.getElementById('bf-points');
